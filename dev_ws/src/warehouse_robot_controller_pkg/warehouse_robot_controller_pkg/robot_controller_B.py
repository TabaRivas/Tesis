...
class Controller(Node):
  ...
  def __init__(self):
    ...
    #The subscription to the movements of robot A is created,
    # to know when robot B has to start
    #The type of message is the same as the previous subscription
    self.subscription_RA = self.create_subscription(
                    Float64MultiArray,
                    '/robot_A/state_est',
                    self.state_robot_A,
                    10)
    self.subscription_RA # prevent unused variable warning
    ...
    #Robot B is notified when robot A enters the room
    self.robot_A_entro = 0
   ...
   # This function tells robot B that the message from 
   # robot A arrives in order to start its journey
   # recorro = 1 the tour begins
   # recorro = o You are not yet enabled to make your tour

  def state_robot_A(self,msg):
    if msg.data[0] > 3:
      self.robot_A_entro = 1   
      
    if msg.data[0] < 3 and self.robot_A_entro == 1:
      self.recorro = 1    
    else:
      self.recorro = 0   
  ...
  def follow_wall(self):
    ...
    msg.angular.y = 0.0
    msg.angular.z = 0.0
    # When he enters the room he starts with the logic of 
    # going through the room glued to the walls
    if self.current_x > 3:
      ...
      self.entreCuarto = 0
      # This if is used to make the robot go directly to the room
      if self.mover_robot < 5:
        self.wall_following_state = "turn left"
        msg.angular.z = -self.turning_speed_wf_fast
        self.mover_robot = self.mover_robot +1      
      else:
        if self.recorro == 1:
          msg.linear.x = self.forward_speed
