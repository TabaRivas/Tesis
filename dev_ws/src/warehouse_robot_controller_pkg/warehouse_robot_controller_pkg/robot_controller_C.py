...
class Controller(Node):
  ...
  def __init__(self):
    ...
    # over a topic named: /robot_C/state_est
    ...
    self.subscription_RB = self.create_subscription(
                    ...
                    self.state_robot_B,
                    10)
    self.subscription_RB # prevent unused variable warning
    # Create a subscriber
   ...
   # This function tells robot C that the message from 
   # robot B arrives in order to start its journey
   # recorro = 1 the tour begins
   # recorro = o You are not yet enabled to make your tour

  def state_robot_B(self,msg):
    if msg.data[0] > 3:
      self.robot_B_entro = 1   
      
    if msg.data[0] < 3 and self.robot_B_entro == 1:
      self.recorro = 1    
    else:
      self.recorro = 0   
